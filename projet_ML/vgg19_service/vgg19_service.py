...
train_datagen = data_gen.flow_from_directory(
    path, target_size=(300, 300), batch_size=16, class_mode='categorical', subset='training'
)
val_datagen = data_gen.flow_from_directory(
    path, target_size=(300, 300), batch_size=16, class_mode='categorical', subset='validation'
)

# Define model architecture with reduced dense layer units
output = model.layers[-1].output
model_final = tf.keras.layers.Flatten()(output)
model_final = tf.keras.layers.Dense(256, activation='relu')(model_final)
model_final = tf.keras.layers.Dense(32, activation='relu')(model_final)
model_final = tf.keras.layers.Dense(10, activation='softmax')(model_final)
model = tf.keras.models.Model(model.input, model_final)

# Compile the model with reduced learning rate and categorical crossentropy loss
